...
#define INITIAL_PREVIEW_HEIGHT 640
#define INITIAL_PREVIEW_WIDTH 480
...
#define DEFAULT_FIXED_FPS_VALUE 15
/*
...
static camera_antibanding_type camera_get_location(void) {
    ...
    ALOGV("value:%s, country value:%s, country code:%d\n",
...
void QualcommCameraHardware::initDefaultParameters()
{
    ...
    if (!parameter_string_initialized) {
        ...
        //Currently Enabling Histogram for 8660
        if(mCurrentTarget == TARGET_MSM8660) {
 ...
 if( mCfgControl.mm_camera_is_supported(CAMERA_PARM_FPS_MODE)){
    ...
    ALOGV("%s: setting num-snaps-per-shutter to %d", __FUNCTION__, numCapture);
    if(mIs3DModeOn)
...
bool QualcommCameraHardware::startCamera()
{
    ...
    //ALOGV("loading libcamera at %p", libmmcamera);
    if (!libmmcamera) {
        ALOGE("FATAL ERROR: could not dlopen liboemcamera.so: %s", dlerror());
...
{	
	ALOGE("bool static native_start_ops: type %d value %d",
            type,value);
	if (type==2) {ALOGE("native_start_ops: type %d value %d",
            type,value);
...
{		
     ALOGE("bool static native_stop_ops: type %d value %d",
            type,value);
...
{
    ALOGV("native_set_parms(%d %d %d)",type,length,*(int *)value);
    ...
    mm_camera_parm_type_t type, uint16_t length, void *value, int *result)
{
    ...
    ALOGV("native_set_parms2 status = %d", status);
...
#undef PARSE_LOCATION
    ...
    if (encode_location) {
        ALOGV("setting image location ALT %d LAT %lf LON %lf",
...
// TODO do properly
	...
	if (mRecordfd[cnt] > 0) {
		...
		MSM_PMEM_VIDEO,
		false, false);
	    ALOGE("Came back from register call to kernel2749");
	  }
#endif
             type = MSM_PMEM_VIDEO;
     ...
     //   ssize_t offset = offset_addr / mPreviewHeap->mAlignedBufferSize;
        ...
        mInPreviewCallback = true;
         ...
         if ((mCurrentTarget == TARGET_MSM8660)&&(mFirstFrame == true)) {
             ALOGV(" receivePreviewFrame : first frame queued, display heap being deallocated");
...
void QualcommCameraHardware::runHFRThread(void *data)
{
    ...
    mInHFRThread = true;
    CAMERA_HAL_UNUSED(data);
    ALOGV("%s: stopping Preview", __FUNCTION__);
    stopPreviewInternal();
                                                   ...
                                                   bool initJpegHeap, int snapshotFormat)
{
    ...
    ALOGV("createSnapshotMemory(%d %d %d %d)",numberOfRawBuffers,numberOfJpegBuffers,initJpegHeap,snapshotFormat);
			...
			if(retVal == NO_ERROR) {
                ...
                if(handle) {
                  ...
                  register_buf(mBufferSize,
                             ...
                             MSM_PMEM_PREVIEW,
                             active);
                  ALOGE("Came back from register call to kernel4368");
                } else
...
void QualcommCameraHardware::stopPreviewInternal()
{
    ...
    if(mIs3DModeOn) {
        ...
        while (mVideoThreadRunning) {
            ALOGV("%s: waiting for video thread to complete.", __FUNCTION__);
            mVideoThreadWait.wait(mVideoThreadWaitLock);
            ALOGV("%s : video thread completed.", __FUNCTION__);
        }
        mVideoThreadWaitLock.unlock();
    }
...
*/
    ...
    else ALOGV("stopPreviewInternal: Preview is stopped already");
...
QualcommCameraHardware* QualcommCameraHardware::createInstance()
{
    ...
    if (singleton != 0) {
        ...
        if (hardware != 0) {
            ALOGV("createInstance: X return existing hardware=%p", &(*hardware));
            singleton_lock.unlock();
            return hardware;
        }
    }
#endif
    {
        ...
        if (rc < 0) {
            ALOGV("createInstance: X failed to create hardware: %s", strerror(errno));
            singleton_lock.unlock();
            return NULL;
        }
    }
...
bool QualcommCameraHardware::initRecord()
{
    ...
    ALOGI("initRecord: mDimension.video_width = %d mDimension.video_height = %d",
             mDimension.video_width, mDimension.video_height);
    // for 8660 the Encoder expects the CbCr offset should be aligned to 2K.
    if(mCurrentTarget == TARGET_MSM8660) {
    ...
    ALOGE("%s  Record fd is %d ", __func__, mRecordfd[cnt]);
        ...
        ALOGE("Registering buffer %d with kernel",cnt);
                  register_buf(mRecordFrameSize,
                             ...
                             type,
                             active);
                  ALOGE("Came back from register call to kernel6346");
    }
...
//    cropp =postviewframe->cropinfo;
    ...
    if(mSnapshotFormat == PICTURE_FORMAT_JPEG) {
        ...
        mDisplayLock.lock();
        int index = 0;//mapThumbnailBuffer(postviewframe);
...
status_t QualcommCameraHardware::setRecordSize(const CameraParameters& params)
{
    ...
    if(!recordSize) {
        ...
        //Extract the record witdh and height that application requested.
        ALOGV("%s: requested record size %s", __FUNCTION__, recordSize);
        if(!parse_size(recordSize, videoWidth, videoHeight)) {
...
status_t QualcommCameraHardware::setPreviewFormat(const CameraParameters& params) {
    ...
    ALOGI("Getting preview format previewformat %d",previewFormat);
	ALOGI("Getting preview format preview_formats %d",preview_formats);
	ALOGI("Getting preview format str %s",str);
	//ALOGI("Getting preview format str_map %d",str_map);

	if(previewFormat != NOT_FOUND) {
		if(!mCfgControl.mm_camera_is_supported(CAMERA_PARM_PREVIEW_FORMAT)) {
        ALOGI("Set preview format not supported for this sensor");
        return NO_ERROR;}
        ...
        if(HAL_currentCameraMode != CAMERA_MODE_3D) {
            ...
            bool ret = native_set_parms(CAMERA_PARM_PREVIEW_FORMAT, sizeof(previewFormat),
                                       (void *)&previewFormat);
			ALOGI("Setting preview format ret %s",ret);
        }else{
...
status_t QualcommCameraHardware::updateFocusDistances(const char *focusmode)
{
    ...
    if( mCfgControl.mm_camera_get_parm(CAMERA_PARM_FOCUS_DISTANCES,
        ...
        if(strcmp(focusmode, CameraParameters::FOCUS_MODE_INFINITY) == 0)
            ...
            snprintf(buffer, sizeof(buffer), ",%f", focusDistances.focus_distance[3]);
        str.append(buffer);
    ...
    ALOGI("%s: mansetting KEY_FOCUS_DISTANCES as %s", __FUNCTION__, "0.010000,0.020000,0.030000");
        mParameters.set(CameraParameters::KEY_FOCUS_DISTANCES, "0.010000,0.020000,0.030000");
...
//    bool all_chnls = false;
    ...
    if (pmemHeap->getHeapID() >= 0) {
        ...
        ALOGV("pmem pool %s ioctl(fd = %d, PMEM_GET_SIZE) is %ld",
             ...
             mFd,
             mSize.len);
        ALOGV("mBufferSize=%d, mAlignedBufferSize=%d\n", mBufferSize, mAlignedBufferSize);
